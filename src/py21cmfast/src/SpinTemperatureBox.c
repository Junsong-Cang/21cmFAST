    ...
    { // This Try{} wraps the whole function.
        ...
        else
        {
            ...
            else
            {
                ...
                if (astro_params->X_RAY_Tvir_MIN < 9.99999e3)
                { // neutral IGM
                    mu_for_Ts = 1.22;
                }
                else
...
#pragma omp for reduction(+ : x_e_ave, Tk_ave)
                ...
                // I don't see any point in doing this because they are gonna be set to 0 later
                        ...
                        for (i = 0; i < user_params->HII_DIM; i++)
                        {
                            for (j = 0; j < user_params->HII_DIM; j++)
                            {
                                for (k = 0; k < user_params->HII_DIM; k++)
                                {
                                    ...
                                    if (flag_options->FIX_VCB_AVG)
                                    { // with this flag we ignore reading vcb box
                                    ...
                                    else
                                    {
                                        ...
                                        { // set vcb to a constant, either zero or vavg.
                                            curr_vcb = 0.0;
                                        }
                                    }
...
#pragma omp for
            ...
            if (filling_factor_of_HI_zp > 1)
                filling_factor_of_HI_zp = 1;
                ...
                if (user_params->USE_INTERPOLATION_TABLES)
                {
                    if (flag_options->USE_MINI_HALOS)
                    {
                        lower_int_limit = fmax(nu_tau_one_MINI(zp, zpp, x_e_ave, filling_factor_of_HI_zp,
                                                               log10_Mcrit_LW_ave, LOG10_MTURN_INT),
                                               (astro_params->NU_X_THRESH) * NU_over_EV);
                    }
                    else
                    {
                    ...
                    if (filling_factor_of_HI_zp < 0)
                        filling_factor_of_HI_zp = 0; // for global evol; nu_tau_one above treats negative (post_reionization) inferred filling factors properly
                    ...
                    for (R_ct = 0; R_ct < global_params.NUM_FILTER_STEPS_FOR_Ts; R_ct++)
                    {
                        ...
                        if (flag_options->USE_MINI_HALOS)
                        {
                            lower_int_limit = fmax(nu_tau_one_MINI(zp, zpp_for_evolve_list[R_ct], x_e_ave, filling_factor_of_HI_zp,
                                                                   log10_Mcrit_LW_ave_list[R_ct], LOG10_MTURN_INT),
                                                   (astro_params->NU_X_THRESH) * NU_over_EV);
                        }
                        else
                        {
...
#pragma omp for
                    for (box_ct = 0; box_ct < HII_TOT_NUM_PIXELS; box_ct++)
                    {
                        // Initializing the box before integrating over zpp
                            ...
                            if (R_ct == 0)
                            {
                                ...
                                { // Must use WF effect
                                    ...
                                    while (fabs(TS_fast - TSold_fast) / TS_fast > 1.0e-3)
                                    {
                                        ...
                                        TS_fast = (1.0 + xa_tilde_fast + xc_fast) * pow(Trad_inv + xa_tilde_fast * (T_inv + 0.405535 * T_inv * pow(TS_fast, -1.) - 0.405535 * T_inv_sq) + xc_fast * T_inv, -1.);
                                    }
                                }
                                else
                                { // Collisions only
                                    // TS_fast = (1.0 + xc_fast) / (Trad_fast_inv + xc_fast * T_inv);
                                    TS_fast = (1.0 + xc_fast) / (Trad_inv + xc_fast * T_inv);
...
#pragma omp for reduction(+ : J_alpha_ave, xalpha_ave, Xheat_ave, Xion_ave, Ts_ave, Tk_ave, x_e_ave)
            ...
            if (LOG_LEVEL >= DEBUG_LEVEL)
            {
                ...
                if (flag_options->USE_MINI_HALOS)
                {
                    ...
                    LOG_DEBUG("zp = %e Ts_ave = %e x_e_ave = %e Tk_ave = %e J_alpha_ave = %e(%e) xalpha_ave = %e \
                          Xheat_ave = %e(%e) Xion_ave = %e J_LW_ave = %e (%e) ",
                              zp, Ts_ave, x_e_ave, Tk_ave, J_alpha_ave,
        ...
        if (cleanup)
            free_TsCalcBoxes(user_params, flag_options);
